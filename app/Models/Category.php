...
class Category extends Model
{
     public static function get_categories(){
		$plans = [];
		$categories = Category::with('sub_categories')->select('id','category_name','category_url','description','image')->where('parent_id',NULL)->where('status','1')->orderby('sortorder','asc')->get()->toArray();
		return $categories;
	}
