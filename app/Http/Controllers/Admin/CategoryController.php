...
class CategoryController extends Controller
{
            ...
            return DataTables::of($query)
               ->addColumn('image', function ($category) {
                    if(!empty($category->image)){	
						return '<img src="'.asset('front/assets/images/category/'.$category->image).'" style="width:50%">';	
					}else{
						return '';
					}
				...
				->addColumn('status', function ($user) use ($usersModule) {
                    ...
                    return '<a class="updateStatus" id="status-'.$user['id'].'" data-url="'.route('updateStatus',['categories',$user['id']]).'" data-id="'.$user['id'].'" style="color:'.$toggleColor.'" href="javascript:void(0)"> 
    ...
    public function addEditCategory(Request $request,$id='')
    {
			...
			if(!empty($id)){
				...
				$previous_id ='';
				if($id> 1){
					$get_previous_id = Category::select('id')->where('id','<',$id)->where('is_delete','0')->orderby('id','desc')->first(); 
				...
				$next_id ='';
				$get_next_id = Category::select('id')->where('id','>',$id)->where('is_delete','0')->orderby('id','asc')->first(); 
    ...
    { 
	     if($request->ajax()){ 
			...
			if(isset($data['id']) && !empty($data['id'])){ 
				$id = $data['id'];
				$category_url_validation ='required|unique:categories,category_url,'.$id;
			}else{
				$category_url_validation = 'required|unique:categories,category_url';
			} 
			$rules = [
                'parent_id' => 'required',
				'category_name' => 'required',
				'category_url' => $category_url_validation,
			...
			if($validator->fails()){
				...
				if($request->hasFile('image')){
                    ...
                    $imageName = 'category-'.rand(1111,999999).'.'.$extension;
